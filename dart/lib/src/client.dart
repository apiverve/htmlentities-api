...
class HtmlentitiesValidationException implements Exception {
  final List<String> errors;

  HtmlentitiesValidationException(this.errors);

  @override
  String toString() => 'HtmlentitiesValidationException: ${errors.join("; ")}';
}
...
/// HTML Entities Encoder/Decoder API client.
///
/// For more information, visit: https://apiverve.com/marketplace/htmlentities?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [html] (required) - The HTML content to encode
/// * [action] - The action to perform
class HtmlentitiesClient {
  final String apiKey;
  ...
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw HtmlentitiesValidationException(errors);
    }
  }

  /// Execute a request to the HTML Entities Encoder/Decoder API.
  ///
  /// Parameters are validated before sending the request.
  Future<HtmlentitiesResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw HtmlentitiesException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return HtmlentitiesResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw HtmlentitiesException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw HtmlentitiesException('Resource not found');
      } else {
        throw HtmlentitiesException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is HtmlentitiesException) rethrow;
      throw HtmlentitiesException('Request failed: $e');
    }
  }
...
/// Exception thrown by the HTML Entities Encoder/Decoder API client.
class HtmlentitiesException implements Exception {
  final String message;

  HtmlentitiesException(this.message);

  @override
  String toString() => 'HtmlentitiesException: $message';
}
