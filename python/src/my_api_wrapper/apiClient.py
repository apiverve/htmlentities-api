...
class HtmlentitiesAPIClientError(Exception):
...
class ValidationError(HtmlentitiesAPIClientError):
...
class HtmlentitiesAPIClient:
    ...
    VALIDATION_RULES = {"html": {"type": "string", "required": True}, "action": {"type": "string", "required": False, "default": "encode"}}
    ...
    def __init__(self, api_key: str, secure: bool = True, debug: bool = False):
        """
        Initialize the HtmlentitiesAPIClient with the API key and optional settings

        :param api_key: Your APIVerve API key
        :param secure: Deprecated. Always set to True
        :param debug: Enable debug logging (default: False)
        :raises HtmlentitiesAPIClientError: If API key is invalid
        """
    ...
    def _validate_api_key(self, api_key: str) -> None:
        ...
        if not api_key or not api_key.strip():
            raise HtmlentitiesAPIClientError(
    ...
    def _log(self, message: str) -> None:
        ...
        """
        if self.debug:
            print(f"[HtmlentitiesAPIClient DEBUG] {message}")
    ...
    def execute(self, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        :raises HtmlentitiesAPIClientError: If the request fails or returns an error
        ...
        try:
            ...
            raise HtmlentitiesAPIClientError(f"Request timeout: {str(e)}")
        except requests.exceptions.ConnectionError as e:
            self._log(f"Connection error: {str(e)}")
            raise HtmlentitiesAPIClientError(f"Connection error: {str(e)}")
        except requests.exceptions.RequestException as e:
            self._log(f"Request exception: {str(e)}")
            raise HtmlentitiesAPIClientError(f"Request failed: {str(e)}")
